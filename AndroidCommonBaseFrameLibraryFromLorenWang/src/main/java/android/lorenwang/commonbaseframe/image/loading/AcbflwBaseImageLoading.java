...
public abstract class AcbflwBaseImageLoading {
     ...
     * 默认加载配置
     */
    protected final AcbflwImageLoadConfig defaultConfig = new AcbflwImageLoadConfig.Build().build();
    ...
    private int getPathOrResType(Object pathOrRes) {
        if (pathOrRes != null) {
            if (pathOrRes instanceof String) {
                if (((String) pathOrRes).matches(
                        JtlwMatchesRegularCommon.EXP_URL_STR) || ((String) pathOrRes).matches(
     ...
     * @param config    回调
     */
    public abstract void loadingNetImage(String path, ImageView imageView,
            @NotNull AcbflwImageLoadConfig config);
     ...
     * @param config    回调
     */
    public abstract void loadingLocalImage(String path, ImageView imageView,
            @NotNull AcbflwImageLoadConfig config);
     ...
     * @param imageView 图片控件
     */
    public abstract void loadingBitmapImage(Bitmap bitmap, ImageView imageView,
            @NotNull AcbflwImageLoadConfig config);
     ...
     */
    protected void setImageViewLayoutParams(boolean useActualAspectRatio,
            @NotNull ImageView imageView, int showViewWidth, int showViewHeight, int imageWidth,
            int imageHeight) {
        int setViewWidth = 0;
        int setViewHeight = 0;
        ...
        if (showViewWidth > 0 && showViewHeight > 0) {
            ...
            if (useActualAspectRatio) {
                setViewWidth = showViewWidth;
                setViewHeight = Float.valueOf(imageHeight * 1.0f / imageWidth * setViewWidth)
                        .intValue();
            }
        } else if (showViewHeight > 0) {
            if (useActualAspectRatio) {
                setViewHeight = showViewHeight;
                setViewWidth = Float.valueOf(imageWidth * 1.0f / imageHeight * setViewHeight)
                        .intValue();
            }
        }
        if (setViewWidth > 0 && setViewHeight > 0 && (imageView
                .getWidth() != setViewWidth || imageView.getHeight() != setViewHeight)) {
