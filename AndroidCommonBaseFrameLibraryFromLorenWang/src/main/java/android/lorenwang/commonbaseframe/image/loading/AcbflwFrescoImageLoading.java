...
class AcbflwFrescoImageLoading extends AcbflwBaseImageLoading {
     ...
     * @param config       配置信息
     */
    private void loadImage(ImageView imageView, @NotNull ImageRequest imageRequest,
        ...
        if (imageView != null && !config.isLoadGetBitmap()) {
            ...
            try {
                dataSource.subscribe(new BaseBitmapDataSubscriber() {
                    ...
                    @Override public void onCancellation(
    ...
    @Override public void loadingNetImage(String path, ImageView imageView,
        ...
        if (!JtlwCheckVariateUtils.getInstance().isEmpty(path) && !JtlwCheckVariateUtils
                .getInstance().isEmpty(imageView) && imageView instanceof DraweeView) {
            //生成请求
            ImageRequest imageRequest = getImageRequest(imageView,
                    ImageRequestBuilder.newBuilderWithSource(Uri.parse(path)), config);
    ...
    @Override public void loadingLocalImage(String path, ImageView imageView,
        ...
        if (path.contains("content://")) {
            uri = new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).query(path).authority(
                    AcbflwBaseConfig.INSTANCE.getApplicationIdPackageNameCurrent() + ".fileprovider").build();
        } else {
            uri = new Uri.Builder().scheme(UriUtil.LOCAL_FILE_SCHEME).path(path).authority(
                    AcbflwBaseConfig.INSTANCE.getApplicationIdPackageNameCurrent() + ".fileprovider").build();
        }
        ImageRequest request = getImageRequest(imageView,
                ImageRequestBuilder.newBuilderWithSource(uri), config);
        if (request != null) {
            loadImage(imageView, request, config);
        }
    }

    @Override public void loadingResImage(int resId, ImageView imageView,
            @NotNull AcbflwImageLoadConfig config) {
        ImageRequest request = getImageRequest(imageView,
                ImageRequestBuilder.newBuilderWithResourceId(resId), config);
        if (request != null) {
            loadImage(imageView, request, config);
        }
    }
    ...
    @Override public void loadingBitmapImage(Bitmap bitmap, ImageView imageView,
            @NotNull AcbflwImageLoadConfig config) {
        if (!JtlwCheckVariateUtils.getInstance().isEmpty(bitmap) && !JtlwCheckVariateUtils
                .getInstance().isEmpty(imageView) && imageView instanceof SimpleDraweeView) {
            try {
                if (bitmap != null) {
                    Uri uri = Uri.parse(MediaStore.Images.Media
                            .insertImage(AtlwConfig.nowApplication.getContentResolver(), bitmap,
                                    null, null));
                    if (uri != null) {
                        loadImage(imageView, getImageRequest(imageView,
                                ImageRequestBuilder.newBuilderWithSource(uri), config), config);
                    }
                }
            } catch (Exception e) {
     ...
     * 清除存储缓存
     */
    @Override public void clearDiskCache() {
     ...
     * 暂停加载图片
     */
    @Override public void pauseLoading() {
     ...
     * 恢复加载图片
     */
    @Override public void resumeLoading() {
     ...
     * @return 图片请求request
     */
    private ImageRequest getImageRequest(ImageView imageView,
            ImageRequestBuilder imageRequestBuilder, @NotNull AcbflwImageLoadConfig config) {
        //设置加载的宽高
        if (config.getResizeLoadWidth() > 0 && config.getResizeLoadHeight() > 0) {
            try {
                ResizeOptions resizeOptions = new ResizeOptions(config.getResizeLoadWidth(),
        ...
        //判断是否需要显示缩略图
        if (imageView != null && isShowThumbnail(imageView, config.getShowViewWidth(),
                config.getShowViewHeight())) {
     ...
     * @return 图片加载控制器
     */
    private DraweeController getController(@NotNull ImageRequest imageRequest,
            @NotNull final ImageView imageView, @NotNull final AcbflwImageLoadConfig config) {
        ...
        if (imageView instanceof DraweeView) {
            ...
            builder.setControllerListener(new BaseControllerListener<ImageInfo>() {
                ...
                public void onFinalImageSet(String id, ImageInfo imageInfo, Animatable animatable) {
                    ...
                    if (config.getLoadCallback() != null) {
                        config.getLoadCallback().onSuccess(null, imageInfo.getWidth(),
                                imageInfo.getHeight());
                    }
                    //设置控件样式
                    setImageViewLayoutParams(
                            config.isUseActualAspectRatio() && ((DraweeView) imageView)
                                    .getAspectRatio() == 0, imageView, config.getShowViewWidth(),
                            config.getShowViewHeight(), imageInfo.getWidth(),
                            imageInfo.getHeight());
     ...
     * @return 占位图控制器
     */
    private GenericDraweeHierarchy getHierarchy(ImageView imageView,
            @NotNull AcbflwImageLoadConfig config) {
        if (imageView instanceof SimpleDraweeView) {
            ...
            if (hierarchy == null) {
                GenericDraweeHierarchyBuilder builder = new GenericDraweeHierarchyBuilder(
                        AtlwConfig.nowApplication.getResources());
