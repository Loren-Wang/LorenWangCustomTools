...
class AcbflwGlideImageLoading extends AcbflwBaseImageLoading {
    ...
    private void loadGildeImage(Object pathOrRes, final ImageView imageView,
        ...
        if (config.getLoadCallback() == null) {
            ...
            getBuild(imageView, requestManager.asBitmap().load(pathOrRes), config).into(
                    new Target<Bitmap>() {
                        ...
                        @Override public void onResourceReady(@NonNull Bitmap resource,
                            ...
                            if (config.getLoadCallback() != null) {
                                config.getLoadCallback().onSuccess(resource, resource.getWidth(),
                                        resource.getHeight());
                            }
                        }
    ...
    private <T> RequestBuilder<T> getBuild(ImageView imageView, RequestBuilder<T> builder,
        ...
        if (config.getBitmapTransformations() != null && config
                .getBitmapTransformations().length > 0) {
            builder = builder.transform(config.getBitmapTransformations());
        }
        //是否显示缩略图
        if (isShowThumbnail(imageView,
                config.getShowViewWidth() > 0 ? config.getShowViewWidth() : null,
        ...
        //占位图设置
        builder = builder.placeholder(AtlwConfig.imageLoadingLoadResId).error(
