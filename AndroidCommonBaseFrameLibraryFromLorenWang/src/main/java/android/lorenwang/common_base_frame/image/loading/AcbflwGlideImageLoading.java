...
class AcbflwGlideImageLoading extends AcbflwBaseImageLoading {
     ...
     * @param config    配置
     */
    private void loadGildeImage(Object pathOrRes, final ImageView imageView, int width, int height, final AcbflwImageLoadConfig config) {
        RequestManager requestManager = Glide.with(AtlwSetting.nowApplication);
...
//            });
     ...
     * @return 构造器
     */
    private <T> RequestBuilder<T> getBuild(RequestBuilder<T> builder, int width, int height, AcbflwImageLoadConfig config) {
        if (config != null) {
        ...
        //占位图设置
        builder = builder.placeholder(AcbflwBaseConfig.imageLoadingLoadResId)
                .error(AcbflwBaseConfig.imageLoadingFailResId);
        return builder;
    }

    @Override
    public void clearMemoryCache() {
        Glide.get(AtlwSetting.nowApplication).clearMemory();
    }

    @Override
    public void clearDiskCache() {
        Glide.get(AtlwSetting.nowApplication).clearDiskCache();
    }

    @Override
    public void pauseLoading() {
        Glide.with(AtlwSetting.nowApplication).pauseRequests();
    }

    @Override
    public void resumeLoading() {
        Glide.with(AtlwSetting.nowApplication).resumeRequests();
    }
}
