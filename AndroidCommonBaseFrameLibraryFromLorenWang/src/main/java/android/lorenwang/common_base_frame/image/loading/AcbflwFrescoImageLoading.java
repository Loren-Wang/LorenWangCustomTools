...
class AcbflwFrescoImageLoading extends AcbflwBaseImageLoading {
    ...
    @Override
    public void loadingNetImage(String path, ImageView imageView, int width, int height, AcbflwImageLoadConfig config) {
    ...
    @Override
    public void loadingLocalImage(String path, ImageView imageView, int width, int height, AcbflwImageLoadConfig config) {
        if (!JtlwCheckVariateUtils.getInstance().isEmpty(path)
                && !JtlwCheckVariateUtils.getInstance().isEmpty(imageView)
                && imageView instanceof SimpleDraweeView) {
    ...
    @Override
    public void loadingResImage(int resId, ImageView imageView, int width, int height, AcbflwImageLoadConfig config) {
    ...
    public void loadingBitmapImage(Bitmap bitmap, ImageView imageView, int width, int height) {
        if (!JtlwCheckVariateUtils.getInstance().isEmpty(bitmap)
            ...
            try {
                if (bitmap != null) {
                    Uri uri = Uri.parse(MediaStore.Images.Media.insertImage(AtlwSetting.nowApplication.getContentResolver(), bitmap, null, null));
                    if (uri != null) {
     ...
     * @param config       回调
     */
    private void loadFrecoImage(final SimpleDraweeView imageView, ImageRequest imageRequest, final AcbflwImageLoadConfig config) {
        if (imageRequest != null) {
            if (config == null || config.getCallback() == null) {
                ...
                if (draweeController != null) {
                    ...
                    GenericDraweeHierarchy draweeHierarchy = getDraweeHierarchy(imageView, AcbflwBaseConfig.imageLoadingLoadResId, AcbflwBaseConfig.imageLoadingFailResId, config);
                    //设置占位图
     ...
     * @return 占位图控制器
     */
    private GenericDraweeHierarchy getDraweeHierarchy(SimpleDraweeView simpleDraweeView, @DrawableRes Integer loadingResId, @DrawableRes Integer failResId, AcbflwImageLoadConfig config) {
        if (loadingResId == null && failResId == null) {
            ...
            if (hierarchy == null) {
                GenericDraweeHierarchyBuilder builder = new GenericDraweeHierarchyBuilder(AtlwSetting.nowApplication.getResources());
