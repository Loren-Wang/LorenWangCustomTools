 ...
 * @author 王亮（Loren）
 */
public class JtlwLogUtil {
    /**
     * 当前实例
     */
    public static JtlwLogUtil logUtils = new JtlwLogUtil();
    /**
     * 是否打印日志
     */
    ...
    public void logD(String tag, String msg) {
        if (!JtlwCheckVariateUtil.getInstance().isEmpty(tag)
                && !JtlwCheckVariateUtil.getInstance().isEmpty(msg)) {
            Logger logger = getLogger(JtlwLogUtil.class);
            if (logger != null) {
    ...
    public void logD(Class cls, String msg) {
        if (!JtlwCheckVariateUtil.getInstance().isEmpty(msg)) {
