...
public class BlueToothStateReceiver extends BroadcastReceiver {
    ...
    public BlueToothStateReceiver(Context context,@NonNull BlueToothReceiverCallback blueToothReceiverCallback) {
        if(blueToothReceiverCallback == null){
            if(context != null) {
                throw new NullPointerException(context.getApplicationContext().getResources().getString(R.string.broadcast_receiver_hint_callback_is_not_null));
            }else {
...
//                Log.d(TAG,"停止搜索远程蓝牙设备");
            ...
            case ACTION_STATE_CHANGED://连接蓝牙，断开蓝牙
                int state = intent.getExtras().getInt(BluetoothAdapter.EXTRA_STATE,-1);
                //系统蓝牙状态回调
