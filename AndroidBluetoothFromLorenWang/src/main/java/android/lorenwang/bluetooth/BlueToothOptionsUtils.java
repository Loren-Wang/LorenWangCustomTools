...
import android.lorenwang.bluetooth.callback.BlueToothOptionsCallback;
...
public class BlueToothOptionsUtils {
    ...
    private void reConBTDevice(BluetoothDevice bluetoothDevice){
        if(bluetoothDevice != null && !TextUtils.isEmpty(bluetoothDevice.getAddress())){
            ...
            if(isRecon) {
                Log.d(TAG, resources.getString(R.string.connect_bt_device_recon));
            }else {
                Log.d(TAG, resources.getString(R.string.connect_bt_device_start));
            }
    ...
    private BluetoothGattCharacteristic getBTGattCharForUUid(UUID serviceUUid, @NonNull UUID characteristicUUid){
        ...
        if(bluetoothGatt != null && characteristicUUid != null){
            if(serviceUUid != null){
                ...
                if(service != null){
                    Log.d(TAG,resources.getString(R.string.send_order_search_service) + service.toString());
                    characteristics = service.getCharacteristic(characteristicUUid);
                    if (characteristics != null) {
                        Log.d(TAG,resources.getString(R.string.send_order_search_characteristic)
    ...
    public synchronized void enableBlueTooth(){
        ...
        if(bluetoothAdapter != null && !bluetoothAdapter.isEnabled()){
            ...
            try {
                ...
                Log.e(TAG,"直接开启蓝牙失败，需要跳转到设置中开启蓝牙");
    ...
    public synchronized void startScanBTDevice(String deviceName,String deviceAddress){
        ...
        if(bluetoothAdapter != null){
            if(bluetoothAdapter.isEnabled()){
                ...
                //扫描未绑定设备
                scanUnBoundBTDevice();
                Log.d(TAG,resources.getString(R.string.scan_bt_device_start));
            }else {
                //开启设备扫描回调
                if(blueToothOptionsCallback != null){
    ...
    public synchronized void stopScanBTDevice(){
        ...
        Log.d(TAG,resources.getString(R.string.scan_bt_device_stop));
    }
    /**
     * 重置扫描
     */
    ...
    public void sendOrderToBTDeviceRead(UUID serviceUUid, @NonNull UUID characteristicUUid, byte[] optValue){
        ...
        if(bluetoothAdapter != null && bluetoothAdapter.isEnabled()){
            ...
            if(characteristic != null){
                Log.d(TAG,resources.getString(R.string.send_order_characteristic_read_ready));
                characteristic.setValue(optValue);
                boolean state = bluetoothGatt.readCharacteristic(characteristic);
                if(state){
                    Log.d(TAG,resources.getString(R.string.send_order_characteristic_read_success));
                }else {
                    Log.d(TAG,resources.getString(R.string.send_order_characteristic_read_fail));
                    connectBTClose();
                    if(blueToothOptionsCallback != null){
    ...
    public void sendOrderToBTDeviceWrite(UUID serviceUUid, @NonNull UUID characteristicUUid, byte[] optValue){
        ...
        if(bluetoothAdapter != null && bluetoothAdapter.isEnabled()){
            BluetoothGattCharacteristic characteristic = getBTGattCharForUUid(serviceUUid,characteristicUUid);
            if(characteristic != null){
                Log.d(TAG,resources.getString(R.string.send_order_characteristic_write_ready));
                characteristic.setValue(optValue);
                boolean state = bluetoothGatt.writeCharacteristic(characteristic);
                if(state){
                    Log.d(TAG,resources.getString(R.string.send_order_characteristic_write_success));
                }else {
                    Log.d(TAG,resources.getString(R.string.send_order_characteristic_write_fail));
                    connectBTClose();
                    if(blueToothOptionsCallback != null){
    ...
    public void sendOrderToBTDeviceNotify(UUID serviceUUid, @NonNull UUID characteristicUUid){
        ...
        if(bluetoothAdapter != null && bluetoothAdapter.isEnabled()){
            BluetoothGattCharacteristic characteristic = getBTGattCharForUUid(serviceUUid,characteristicUUid);
            if(characteristic != null){
                Log.d(TAG,resources.getString(R.string.send_order_characteristic_notify_ready));
                boolean state = bluetoothGatt.setCharacteristicNotification(characteristic,true);
                if(state){
                    Log.d(TAG,resources.getString(R.string.send_order_characteristic_notify_success));
                    ...
                    Log.d(TAG,resources.getString(R.string.send_order_characteristic_notify_fail));
                    connectBTClose();
                    if(blueToothOptionsCallback != null){
                        blueToothOptionsCallback.onCharacteristicRead(null,null,null);
                    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case PAUSE_SCAN_DEVICE://停止设备扫描并在指定时间之后再次开启扫描
                    if(isScan) {
                        Log.d(TAG,resources.getString(R.string.scan_bt_device_pause));
                        pauseScanUnBoundBTDevice();
                    }
                    break;
                case RESET_SCAN_DEVICE://开始设备扫描并在指定时间之后再次关闭扫描
                    if(isScan) {
                        Log.d(TAG,resources.getString(R.string.scan_bt_device_reset));
                        scanUnBoundBTDevice();
                    }
                    break;
    ...
    private BlueToothReceiverCallback blueToothReceiverCallback = new BlueToothReceiverCallback() {
        ...
        public void systemBlueToothStateChange(Integer state) {
            switch (state){
                case BluetoothAdapter.STATE_OFF:
                    Log.d(TAG, resources.getString(R.string.system_bluetooth_off));
                    if(blueToothOptionsCallback != null){
                        blueToothOptionsCallback.systemBTDeviceStateChange(false);
                    }
                    break;
                case BluetoothAdapter.STATE_ON:
                    Log.d(TAG,resources.getString(R.string.system_bluetooth_on));
                    if(blueToothOptionsCallback != null){
                        blueToothOptionsCallback.systemBTDeviceStateChange(true);
                    }
                    break;
                case BluetoothAdapter.STATE_TURNING_OFF:
                    Log.d(TAG,resources.getString(R.string.system_bluetooth_turning_off));
                    connectBTClose();
                    stopScanBTDevice();
                    break;
                case BluetoothAdapter.STATE_TURNING_ON:
                    Log.d(TAG,resources.getString(R.string.system_bluetooth_turning_on));
                    break;
                default:
                    break;
            }
        ...
        public void scanFoundBlueToothDevice(BluetoothDevice bluetoothDevice) {
            if(bluetoothDevice != null && !TextUtils.isEmpty(bluetoothDevice.getAddress())){
                Log.d(TAG,resources.getString(R.string.scan_bt_device_address) + bluetoothDevice.getAddress());
                if(blueToothOptionsCallback != null){
