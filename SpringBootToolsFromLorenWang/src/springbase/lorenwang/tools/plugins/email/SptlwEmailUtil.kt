 ...
 * 备注：
 */
open class SptlwEmailUtil(config: Config) {
    /**
     * 邮件发送者
     */
    ...
    init {
        try {
            ...
            sptlwConfig.getLogUtil().logI(this::class.java, "邮件发送工具初始化结束，测试连接状态")
        } catch (e: Exception) {
            javaMailSender = null
            sptlwConfig.getLogUtil().logE(this::class.java, "邮件发送工具初始化失败，失败原因：${e.message}")
        }
    }
    ...
    fun sendEmailMessage(title: String, content: String, toEmail: String): Boolean {
        return javaMailSender.kttlwEmptyCheck({
            sptlwConfig.getLogUtil().logI(this::class.java, "向${toEmail}发送邮件失败,邮件发送实例无法初始化")
            false
        }, {
            try {
                sptlwConfig.getLogUtil().logI(this::class.java, "开始向${toEmail}发送邮件")
                ...
                it.send(message)
                sptlwConfig.getLogUtil().logI(this::class.java, "向${toEmail}发送邮件成功")
                true
            } catch (e: Exception) {
                sptlwConfig.getLogUtil().logI(this::class.java, "向${toEmail}发送邮件失败：${e.message}")
                false
            }
        })
    }
