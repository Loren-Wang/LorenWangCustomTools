...
import springbase.lorenwang.tools.plugins.oss.aliyun.SptlwALiYunOssUtil
import springbase.lorenwang.tools.plugins.oss.qiniu.SptlwQiNiuOssConfig
import springbase.lorenwang.tools.plugins.oss.qiniu.SptlwQiNiuOssUtil
 ...
 * 备注：
 */
class SptlwOssUtil private constructor() : AbstractOssOptions() {
    /**
     * oss操作实例
     */
    ...
    companion object {
        @Volatile
        private var optionsInstance: SptlwOssUtil? = null

        @JvmStatic
        val instance: SptlwOssUtil
            get() {
                if (optionsInstance == null) {
                    synchronized(this::class.java) {
                        if (optionsInstance == null) {
                            optionsInstance = SptlwOssUtil()
                        }
                    }
                }
    ...
    fun initOssConfig(ossConfig: Any) {
        when (ossConfig) {
            is SptlwALiYunOssConfig -> {
                ossOptions = SptlwALiYunOssUtil(ossConfig)
            }
            is SptlwQiNiuOssConfig -> {
                ossOptions = SptlwQiNiuOssUtil(ossConfig)
            }
        }
    }
