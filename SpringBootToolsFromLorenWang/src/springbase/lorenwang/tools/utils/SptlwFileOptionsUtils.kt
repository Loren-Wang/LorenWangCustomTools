 ...
 * 备注：子类必须将构造函数私有化
 */
abstract class SptlwFileOptionsUtils {
    /**
     * 文件最大大小，单位是字节
     */
     ...
     * 初始化配置
     */
    fun initConfig(ossTypeFileMaxSize: Long) {
    ...
    fun checkFileStatus(file: MultipartFile, receiveFileTypes: Array<JtlwFileTypeEnum>,
        ...
        if (fileHeader.isNullOrEmpty()) {
            SpblwConfig.logUtils.logE(this.javaClass, "文件${
        ...
        //上传文件空判断
        if (file.isEmpty) {
            SpblwConfig.logUtils.logE(this.javaClass, "文件${file.originalFilename}---上传文件为空，无法执行上传流程！", true)
            return getFileEmptyStatusBean()
        }
        //判断文件上传大小
        if (file.size > ossTypeFileMaxSize) {
            SpblwConfig.logUtils.logE(this.javaClass, "文件${file.originalFilename}---上传文件过大，大于${ossTypeFileMaxSize}！", true)
            return getFileTooLargeStatusBean()
        }
        //文件接收类型判断
        if (receiveFileTypes.isNotEmpty() && !receiveFileTypes.contains(getFileType(fileHeader))) {
            SpblwConfig.logUtils.logE(this.javaClass, "文件${file.originalFilename}---不是接收类型数据！", true)
