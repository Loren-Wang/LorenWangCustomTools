...
import springbase.lorenwang.base.database.SpblwDatabaseParams.FIRST_RANK_LIST
 ...
 */
fun <CURD : CrudRepository<TB, ID>, ID, TB : SpblwBaseTb, R : SpblwBaseHttpServletRequestWrapper, BC : SpblwBaseController<R>> CURD.spblwUpDataTbAllRank(
    baseController: BC, rankBean: KttlwBaseNetUpDateRankReqBean<ID>, checkOldCount: Boolean,
    getNewSaveTbFun: (tbInfo: TB, newRank: Long) -> TB): String {
    //做数量判断
 ...
 *
 */
fun <CURD : CrudRepository<TB, ID>, ID, TB : SpblwBaseTb, R> CURD.spblwModifyOptions(zeroError: Boolean, options: (CURD) -> Int, successFun: () -> R,
    errorFun: (Exception?) -> R): R {
    return try {
 ...
 */
fun <CURD : CrudRepository<TB, ID>, ID, TB : SpblwBaseTb, R> CURD.spblwFindTbInfoOptions(options: (CURD) -> TB?, successFun: (TB) -> R,
    errorFun: (Exception?) -> R): R {
    return try {
        val result = options(this)
        if (result != null) {
