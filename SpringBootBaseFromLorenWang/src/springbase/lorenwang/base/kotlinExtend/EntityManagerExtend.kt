...
fun EntityManager.spblwUpDataAllRankMove(tableName: String, primaryKeyColumn: String, rankColumn: String?, fromRank: BigInteger,
    ...
    val list: MutableList<Any?> = when {
        ...
        fromRank > newRankOptions -> {
            ...
            this.createNativeQuery(
                "select $primaryKeyColumn from $tableName where $rankColumn>=$newRankOptions and $rankColumn<$fromRank order by $rankColumn").resultList
        }
        fromRank < newRankOptions -> {
            changeRank = BigInteger.valueOf(-1)
            startRank = fromRank + BigInteger.ONE
            this.createNativeQuery(
                "select $primaryKeyColumn from $tableName where $rankColumn>$fromRank and $rankColumn<=$newRankOptions order by $rankColumn").resultList
        }
        else -> {
    ...
    for (index in 0 until size) {
        ...
        primaryKey?.let {
            if (this.createNativeQuery("update $tableName set $rankColumn=${
 ...
 */
fun EntityManager.spblwRemoveRank(tableName: String, primaryKeyColumn: String, rankColumn: String?, deleteRank: BigInteger): BigInteger? {
    return spblwUpDataAllRankMove(tableName, primaryKeyColumn, rankColumn, deleteRank, BigInteger.ZERO)
}
 ...
 */
fun <ID> EntityManager.spblwDeleteTableInfo(tableName: String, primaryKeyColumn: String, rankColumn: String?, id: ID): Boolean {
    ...
    return (this.createQuery("delete $tableName  where $primaryKeyColumn=$id").executeUpdate() <= 0).let {
        if (it) {
            ...
            //删除成功，判断是否需要移除排行
            if (deleteRank != null) {
                spblwRemoveRank(tableName, primaryKeyColumn, rankColumn, deleteRank!!)
            }
            true
        }
    }
 ...
 */
fun EntityManager.spblwGetTableMaxRank(tableName: String, rankColumn: String): BigInteger? {
